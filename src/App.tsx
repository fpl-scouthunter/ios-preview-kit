...
export default function App() {
  ...
  const handleExportAll = async () => {
    if (previewRef.current === null || screens.length === 0) return;

    setIsExporting(true);
    setExportProgress(0);
    const zip = new JSZip();
    const originalId = activeScreenId;
    ...
    try {
      for (let i = 0; i < screens.length; i++) {
        ...
        zip.file(`preview-${i + 1}-${screen.title.slice(0, 10)}.png`, base64Data, { base64: true });
      }

      setExportProgress(100);
      const content = await zip.generateAsync({ type: 'blob' });
      saveAs(content, 'ios-app-previews.zip');
    } catch (err) {
